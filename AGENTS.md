...
- **Classes Utilitárias (`globals.css`):** Todas as repetições de Glassmorphism (blur, backgrounds RGBA complexos, bordas translucidas) DEBEM ser abstraídas em `@layer utilities` no `globals.css` (ex: `.glass-modal`, `.glass-overlay`, `.glass-input`).
